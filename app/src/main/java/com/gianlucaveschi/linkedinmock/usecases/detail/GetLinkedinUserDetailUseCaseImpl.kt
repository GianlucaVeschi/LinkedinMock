...
class GetLinkedinUserDetailUseCaseImpl(
    ...
    override suspend fun run(uid: Int): Flow<DataState<LinkedinUser>> = flow {
        emit(DataState.loading())
        emit(getLinkedinUserDetail(uid))
    }

    private suspend fun getLinkedinUserDetail(uid: Int): DataState<LinkedinUser> = try {
    ...
    private fun handleError(exceptionMessage: String?): DataState<LinkedinUser> {
